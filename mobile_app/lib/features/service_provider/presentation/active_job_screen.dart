...
class _ActiveJobScreenState extends ConsumerState<ActiveJobScreen> {
  ...
  final List<String> _steps = ['En Route', 'Arrived', 'In Progress', 'Completed'];

  void _advanceStep() {
    if (_currentStep < 3) {
      setState(() => _currentStep++);
    } else {
      Navigator.pushReplacementNamed(context, AppRoutes.providerComplete.replaceAll(':id', widget.jobId));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Active Job', style: TextStyle(color: AppColors.textPrimary)),
                  ...
                  return Expanded(
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: index <= _currentStep ? AppColors.primary : Colors.white,
                        borderRadius: BorderRadius.circular(4),
                        border: Border.all(color: index <= _currentStep ? AppColors.primary : AppColors.border),
                      ),
                    ),
                  );
                }),
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  Text(_steps[_currentStep], style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.primary)),
                  ...
                  if (_currentStep == 2) ...[
                    const Text('Job Checklist', style: TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(
                        _currentStep == 0 ? 'Mark Arrived' 
                        : _currentStep == 1 ? 'Start Job' 
                        : _currentStep == 2 ? 'Review & Complete' : 'Finish',
  ...
  Widget _buildCheckItem(String text) {
    return Padding(
      ...
      child: Row(
        children: [
          const Icon(Icons.circle_outlined, color: AppColors.inactive, size: 18),
          const SizedBox(width: 8),
          Text(text),
        ],
      ),
